...
import { auth, signIn, signUp, logout, getCurrentToken, PersonaAttributes } from '../services/firebase/auth';
...
interface AuthContextType {
  ...
  signUp: (email: string, password: string, persona: string, personaAttrs: PersonaAttributes) => Promise<void>;
...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  const handleSignUp = async (email: string, password: string, persona: string, personaAttrs: PersonaAttributes) => {
    const { user, idToken } = await signUp(email, password, persona, personaAttrs);
    setUser(user);
    setIdToken(idToken);
  };
