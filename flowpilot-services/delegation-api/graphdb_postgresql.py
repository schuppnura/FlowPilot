...
class DelegationGraphDB:
    ...
    def _ensure_schema(self) -> None:
        ...
        with self._get_connection() as conn:
            with conn.cursor() as cursor:
                ...
                # Drop old unique constraint with scope if it exists (migration from old schema)
                cursor.execute("""
                    DROP INDEX IF EXISTS idx_active_delegation_unique
                """)
                
                # Create partial unique index WITHOUT scope - only one active delegation allowed
                # per (principal, delegate, workflow) tuple, regardless of scope
                # This allows re-creating delegations after revocation
                cursor.execute("""
                    CREATE UNIQUE INDEX IF NOT EXISTS idx_active_delegation_unique_no_scope
                    ON delegations(principal_id, delegate_id, COALESCE(workflow_id, ''))
    ...
    def insert_edge(
        ...
        # If delegation exists with same scope/expires_at, returns existing (idempotent).
        # If delegation exists with different scope/expires_at, raises ValueError.
        # Args:
        ...
        # Returns:
        #     Tuple of (delegation_dict, was_created: bool)
        #     was_created is True if newly created, False if already existed
        self._ensure_schema()

        if scope is None:
            scope = ["execute"]
        scope_json = json.dumps(sorted(scope))  # Sort for consistent comparison

        with self._get_connection() as conn:
            with conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor) as cursor:
                # Check if delegation already exists (regardless of scope)
                cursor.execute("""
                ...
                if existing_row:
                    # Delegation exists - check if params match
                    existing_scope_json = existing_row["scope"]
                    try:
                        existing_scope = json.loads(existing_scope_json) if existing_scope_json else ["execute"]
                        existing_scope_normalized = json.dumps(sorted(existing_scope))
                    except (json.JSONDecodeError, TypeError):
                        existing_scope = ["execute"]
                        existing_scope_normalized = json.dumps(["execute"])
                    
                    # Compare normalized scopes and expires_at
                    existing_expires = existing_row["expires_at"].isoformat()
                    if existing_scope_normalized == scope_json and existing_expires == expires_at:
                        # Identical delegation - return existing (idempotent)
                        row = existing_row
                        was_created = False
                    else:
                        ...
                        raise ValueError(
                            ...
                            f"with different parameters (scope: {existing_scope}, expires: {existing_expires}). "
                            f"To modify, first revoke using DELETE /v1/delegations, then create a new one."
                        )
                else:
                ...
                return (delegation_dict, was_created)
