...
def _fetch_user_by_id(user_sub: str) -> Optional[Dict[str, Any]]:
    ...
    if not token:
        print(
            "[profile] Service token not available - check KEYCLOAK_TOKEN_URL, AGENT_CLIENT_ID, AGENT_CLIENT_SECRET",
            flush=True,
        )
        return None
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(
                f"[profile] Failed to fetch user {user_sub}: HTTP {response.status_code}: {response.text}",
                flush=True,
            )
            return None
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if isinstance(users, list):
                ...
                print(
                    f"[profile] Unexpected response format: {type(users)}", flush=True
                )
                return []
        else:
            print(
                f"[profile] Failed to fetch users: HTTP {response.status_code}: {response.text}",
                flush=True,
            )
            return []
    except Exception as e:
