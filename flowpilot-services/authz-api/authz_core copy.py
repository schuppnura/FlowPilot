...
def build_opa_resource(
    ...
    if resource_attributes:
        resource_properties_augmented = normalize_attributes(
    ...
    if owner_id:
        ...
        workflow_id = resource_properties_augmented.get("workflow_id") or request_resource.get("id")
...
def build_opa_context(
    ...
    context_principal = dict(principal) # first, use whatever was in AuthZEN
    ...
    if principal_persona:
        ...
        context_principal["persona_status"] = principal_persona.get("status", "")
        context_principal["persona_valid_from"] = principal_persona.get("valid_from", "")
        context_principal["persona_valid_till"] = principal_persona.get("valid_till", "")
    else:
        # Persona fetch failed - continue with 'unknown' persona
        # This is acceptable as policy-check is done by opa
        context_principal["persona_status"] = "not_found"
        context_principal["persona_valid_from"] = ""
        context_principal["persona_valid_till"] = ""
