...
class PersonaDB:
    ...
    def _init_schema(self) -> None:
        ...
        Uses composite primary key (persona_id = user_sub_title) to enforce uniqueness.
        """
    ...
    def create_persona(
        ...
        **custom_attributes: Any,
        ...
        Uses composite persona ID (user_sub + title) to enforce uniqueness at database level.
        If persona already exists, returns existing persona (idempotent).
        ...
        """
        # Generate composite persona ID from user_sub and title
        ...
        conn = self._get_connection()
        try:
            # Check if persona already exists (idempotency check)
            cursor = conn.execute(
                "SELECT * FROM personas WHERE persona_id = ?", (persona_id,)
            )
            existing_row = cursor.fetchone()
            if existing_row:
                # Persona already exists - return existing (idempotent)
                return self._row_to_dict(existing_row)
            ...
            conn.execute(
                ...
                INSERT INTO personas (
                    persona_id, user_sub, title, scope, valid_from, valid_till,
