...
# Determine deployment environment for database selection
DEPLOYMENT_ENV = os.environ.get("DEPLOYMENT_ENV", "firebase")

# Import appropriate persona database based on deployment environment
if DEPLOYMENT_ENV == "keycloak":
    from personadb_sqlite import PersonaDB
else:
    from personadb_firestore import PersonaDB
...
class UserInfoResponse(BaseModel):
    """User info for delegation candidates."""
    sub: str
    email: str | None
    persona: str
...
@app.get("/v1/users/by-persona", response_model=ListUsersResponse)
def get_users_by_persona(
    ...
    """
    List users who have a specific persona title (compatibility endpoint using new persona system).
    
    This endpoint queries the new persona database to find all users with active personas
    matching the specified title, and enriches results with email addresses from Firebase Auth.
    
    Recommended for web app migration from legacy /v1/users endpoint.
    """
    ...
    try:
        ...
        # Query new persona system for all active personas with this title
        personas = _persona_db.list_personas_by_title(sanitized_title, status="active")

        # Import Firebase Admin to fetch emails
        import firebase_admin
        from firebase_admin import auth as firebase_auth

        # Build user list with email lookup from Firebase Auth
        users = []
        for persona in personas:
            user_sub = persona["user_sub"]
            email = None
            
            # Try to fetch email from Firebase Auth
            try:
                user_record = firebase_auth.get_user(user_sub)
                email = user_record.email
            except Exception as email_error:
                # Log error but continue - user may not exist in Firebase Auth
                print(f"Warning: Could not fetch email for user {user_sub}: {email_error}", flush=True)
        ...
        api_logging.log_api_response("GET", "/v1/users/by-persona", 200, response_body=response)
        return response

    except Exception as exc:
        error_detail = security.sanitize_error_message(str(exc), INCLUDE_ERROR_DETAILS)
        api_logging.log_api_response("GET", "/v1/users/by-persona", 500, error=error_detail)
...
def create_persona(
    ...
    try:
        ...
        # Separate standard attributes from custom attributes
        standard_attrs = {"title", "scope", "valid_from", "valid_till", "status"}
        
        # Extract standard attributes
        create_kwargs = {
            "user_sub": user_sub,
            "title": sanitized_body["title"],
            "scope": sanitized_body.get("scope"),
            "valid_from": sanitized_body.get("valid_from"),
            "valid_till": sanitized_body.get("valid_till"),
        ...
        api_logging.log_api_response("POST", "/v1/personas", 201, response_body=persona)
        return persona

    except security.InputValidationError as exc:
        error_detail = security.sanitize_error_message(str(exc), INCLUDE_ERROR_DETAILS)
        api_logging.log_api_response("POST", "/v1/personas", 400, error=error_detail)
        ...
        error_detail = security.sanitize_error_message(str(exc), INCLUDE_ERROR_DETAILS)
        api_logging.log_api_response("POST", "/v1/personas", 500, error=error_detail)
        raise HTTPException(status_code=500, detail=error_detail) from exc
...
def get_persona(
    ...
    try:
        ...
        api_logging.log_api_response(
            "GET", f"/v1/personas/{persona_id}", 200, response_body=persona
        )
        return persona
        ...
        error_detail = security.sanitize_error_message(str(exc), INCLUDE_ERROR_DETAILS)
        api_logging.log_api_response(
            "GET", f"/v1/personas/{persona_id}", 404, error=error_detail
        )
        raise HTTPException(status_code=404, detail=error_detail) from exc
    except Exception as exc:
        error_detail = security.sanitize_error_message(str(exc), INCLUDE_ERROR_DETAILS)
        api_logging.log_api_response(
            "GET", f"/v1/personas/{persona_id}", 500, error=error_detail
        )
        raise HTTPException(status_code=500, detail=error_detail) from exc
...
def list_personas(
    ...
    api_logging.log_api_request(
        ...
        token_claims=token_claims,
        request=request,
        path_params={"status": status} if status else None,
    )
    ...
    try:
        ...
        api_logging.log_api_response("GET", "/v1/personas", 200, response_body=response)
        return response

    except Exception as exc:
        error_detail = security.sanitize_error_message(str(exc), INCLUDE_ERROR_DETAILS)
        api_logging.log_api_response("GET", "/v1/personas", 500, error=error_detail)
        raise HTTPException(status_code=500, detail=error_detail) from exc
...
def list_personas_for_user(
    ...
    # Check for either Keycloak service account (client_id=flowpilot-agent)
    # or GCP service account (email contains gserviceaccount.com)
    client_id = token_claims.get("azp") or token_claims.get("client_id")
    email = token_claims.get("email", "")
    is_service_account = client_id == "flowpilot-agent" or "gserviceaccount.com" in email
    ...
    try:
        ...
        api_logging.log_api_response(
            "GET", f"/v1/users/{user_sub}/personas", 200, response_body=response
        )
        return response
        ...
        error_detail = security.sanitize_error_message(str(exc), INCLUDE_ERROR_DETAILS)
        api_logging.log_api_response(
            "GET", f"/v1/users/{user_sub}/personas", 500, error=error_detail
        )
        raise HTTPException(status_code=500, detail=error_detail) from exc
...
def update_persona(
    ...
    try:
        ...
        update_kwargs = {
            ...
            "user_sub": user_sub,
            "title": sanitized_body.get("title"),
            "scope": sanitized_body.get("scope"),
            "valid_from": sanitized_body.get("valid_from"),
            "valid_till": sanitized_body.get("valid_till"),
            "status": sanitized_body.get("status"),
        ...
        api_logging.log_api_response(
            "PUT", f"/v1/personas/{persona_id}", 200, response_body=persona
        )
        return persona
        ...
        error_detail = security.sanitize_error_message(str(exc), INCLUDE_ERROR_DETAILS)
        api_logging.log_api_response(
            "PUT", f"/v1/personas/{persona_id}", 400, error=error_detail
        )
        raise HTTPException(status_code=400, detail=error_detail) from exc
    except ValueError as exc:
        error_detail = security.sanitize_error_message(str(exc), INCLUDE_ERROR_DETAILS)
        ...
        error_detail = security.sanitize_error_message(str(exc), INCLUDE_ERROR_DETAILS)
        api_logging.log_api_response(
            "PUT", f"/v1/personas/{persona_id}", 500, error=error_detail
        )
        raise HTTPException(status_code=500, detail=error_detail) from exc
...
def delete_persona(
    ...
    try:
        ...
        api_logging.log_api_response("DELETE", f"/v1/personas/{persona_id}", 204)
        ...
        error_detail = security.sanitize_error_message(str(exc), INCLUDE_ERROR_DETAILS)
        api_logging.log_api_response(
            "DELETE", f"/v1/personas/{persona_id}", 404, error=error_detail
        )
        raise HTTPException(status_code=404, detail=error_detail) from exc
    except Exception as exc:
        error_detail = security.sanitize_error_message(str(exc), INCLUDE_ERROR_DETAILS)
        api_logging.log_api_response(
            "DELETE", f"/v1/personas/{persona_id}", 500, error=error_detail
        )
        raise HTTPException(status_code=500, detail=error_detail) from exc
